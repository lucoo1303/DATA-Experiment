...
# functie om de data van alle video's op een tijdstip t te verzamelen
...
def get_data_at_time(videos, t):
    data_at_t = []
    for vid in videos:
        theta0 = get_theta0(vid)
        theta_tn = theta_and_t_at_tn(vid, t)[1]
        noms = [theta0.n, theta_tn.n]
        stds = [theta0.s, theta_tn.s]
        data_at_t.append(unp.uarray(noms, stds))
    return np.array(data_at_t)
...
def plot_theta_vs_theta0(data):
    ...
    for n, t in enumerate(tn):
        data_at_t = get_data_at_time(data, t)
        theta_0 = data_at_t[:,0]
        theta_t = data_at_t[:,1]
...
def fit_theta_vs_theta0(data):
    ...
    for n, t in enumerate(tn):
        ...
        theta_0 = data_at_t[:,0]
        theta_t = data_at_t[:,1]
        fit_params.append(fit_at_t(ax, theta_0, theta_t, n))
...
# functie om de data van een specifieke video te plotten voor extra data check
